"""Convolutional Neural Network with TensorFlow Exemplo"""
...
def maxpool2d(x):
    window_size = [1,2,2,1]
    window_moviment = [1,2,2,1]
    return tf.nn.max_pool(x, ksize=window_size, strides=window_moviment, padding='SAME')
...
def train_neural_network(x):
    ...
    with tf.Session() as sess:
        ...
        for _ in range(int(mnist.test.num_examples/batch_size)):
            ...
            good += accuracy_sum.eval(feed_dict={x: testSet[0],
                                                 y: testSet[1], 
                                                 keep_prob: 1.0})
